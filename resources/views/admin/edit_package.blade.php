...
<div class="row">
	...
	{!! Form::open(array('route'=> ['pricelist.update', $data->id], 'method'=>'PUT','class'=>'form-horizontal')) !!}

	<table class="table table-stripped">
		
	<tr class="{{ $errors->has('package') ? 'has-error' : '' }}">
		<td>{{ Form::label('package', 'Package Name', array('class'=>'control-label'))}}	</td>
		<td>{{ Form::text('package', $data->package, array('class'=>'col-md-6 form-control', 'placeholder'=>'Package Name'))}}
	...
	<tr class="{{ $errors->has('price') ? 'has-error' : '' }}">
		...
		<td>{{ Form::text('price', $data->price, array('class'=>'col-md-6 form-control', 'placeholder'=>'price'))}}
		@if($errors->has('price'))
		 <span class="help-block" style="display:block">
	...
	<tr class="{{ $errors->has('details') ? 'has-error' : '' }}">
		...
		<td>{{ Form::textarea('details', $data->details, array('class'=>'col-md-6 form-control', 'placeholder'=>'details', 'id'=>'details'))}}
