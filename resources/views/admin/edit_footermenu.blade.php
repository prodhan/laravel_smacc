...
<div class="row">
	...
	{!! Form::open(array('route'=> ['footermenu.update', $data->id], 'method'=>'PUT','class'=>'form-horizontal')) !!}
	...
	<tr>
		...
		<td>{{ Form::text('link', $data->link, array('class'=>'col-md-6 form-control', 'placeholder'=>'Link here'))}}</td>	
	</tr>	
	<tr class="{{ $errors->has('title') ? 'has-error' : '' }}">
		<td>{{ Form::label('position', 'Position', array('class'=>'control-label'))}}	</td>
		<td>{{ Form::select('position',  ['1'=>'Left', '2'=>'Center', '3'=>'Right'], $data->position, array('class'=>'col-md-6 form-control', 'placeholder'=>'Where to display'))}}
